...
void List::Insert()
{
	...
	else // 2. tail 不為空時，ptr往後找
	{
		...
		while(curr != NULL)
		{
			if (curr->num == add->num) // 1. 等於的情況
			{
				printf("%d已經存在串列中\n", num);
				return;
			}
			else if(curr->num < add->num) // 2. 小於的情況(插在 curr 前面) ex. pre->add->curr
			{
				if(head == curr) // a. 如果要插的是 head 前(增加的數字是最大的)
				{
					add->next = head;
					head = add;
					return;
